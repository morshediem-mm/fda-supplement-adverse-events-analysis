...
The purpose of the Process phase is to clean, validate, filter, 
and integrate the FDA FAERS Q2 2025 data to ensure it is accurate, 
consistent, and ready for analysis.

This phase focuses on preparing reliable analytical datasets 
without altering the original raw data.
...
Power Query was used to handle large files, apply transformations 
consistently, and document each processing step.
...
- DRUG25Q2.txt — Drug and product information
- REAC25Q2.txt — Adverse reactions
- OUTC25Q2.txt — Patient outcomes

Other files (INDI, RPSR, THER) remain available for future 
expansion but were not required for the primary analytical questions.
...
This ensures analysis focuses only on pharmaceutical drugs 
identified as the primary suspected cause of the adverse event.

---

## Step 3 — Standardize and Validate Key Fields

**Drug Name Cleaning (DRUGNAME)**
...
Full name normalization is documented but not aggressively merged 
to preserve reporting transparency.
...
**Duplicate Records**
- Duplicate PRIMARYID rows retained where they represent valid 
  multiple reactions or outcomes
...
| File | Description |
|------|-------------|
| DEMO_clean | Cleaned demographic data with validated identifiers |
| DRUG_clean | Filtered primary suspect drugs with standardized names |
| REAC_clean | Reaction-level data linked to drugs |
| OUTC_clean | Outcome data linked to cases |

These files serve as the foundation for analytical calculations 
...
|-------|---------|
| DEMO_clean | 393,131 |
| DRUG_clean | 1,048,575 |
| REAC_clean | 1,048,575 |
| OUTC_clean | 295,583 |
